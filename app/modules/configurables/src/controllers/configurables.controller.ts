...
/**
 * PATCH /api/configurables
 *
 * Updates the configurable_data for this app instance.
 * Used by the parent setup screen to persist routine configuration.
 */
export async function patchConfigurables(req: Request, res: Response) {
  try {
    const { configurable_data } = req.body;
    if (!configurable_data) {
      return res.status(400).json({ message: "configurable_data is required" });
    }
    await ConfigurablesService.updateConfig({ configurable_data });
    return res.json({ success: true });
  } catch (error) {
    console.error("Failed to update configurables:", error);
    return res.status(500).json({ message: "Failed to update configurables" });
  }
}
